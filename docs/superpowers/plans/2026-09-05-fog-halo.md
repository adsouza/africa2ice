...
Verified against the tree: the function is `parseDesktopOptions(args []string, stderr io.Writer)` at `main.go:78`, and `main_test.go` already imports `bytes`, `path/filepath`, and `strings` but **not** `io` — hence `&bytes.Buffer{}` above rather than `io.Discard`. `main_test.go` carries a `//go:build !js` tag; the new tests inherit it.
